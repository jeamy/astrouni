...
extern  SYS     Sys;
extern  double  dKREIS_STZ;   //auhelper.c
extern  double  dKREIS_10;
extern  double  dKREIS_3;
extern  double  dKREIS_5;
extern  double  dKREIS_PLANET;
LOGFONT MainFontRec;
...
// break;
 ...
 case MAUS_ASPEKT:
	 ...
	 Ellipse( hDC, sMpX-sA, sMpY-sA, sMpX+sA, sMpY+sA );
	 if (Radix->sLB&LB_A)
		sDrawAspekte(hDC, (RADIX*)Radix->plRadix[0], Radix, (char)Sys.ScrFactor);
     else
...
///2. Kreis (STZ)****************
 sR=(short)((double)sRadius*dKREIS_STZ);
 sRG=sR;
 Ellipse(hDC, sMpX-sR, sMpY-sR, sMpX+sR, sMpY+sR );
///3. Kreis (10 Grad)****************
 sR=(short)((double)sRadius*dKREIS_10);
 sRI=sR;
 if (auinit.sTeilung&S_3er)
	Ellipse(hDC, sMpX-sR, sMpY-sR, sMpX+sR, sMpY+sR );
...
// +(short)((double)AstroFontRec.lfWidth/2);
 ...
 sRG=sR;
 sR=(short)((double)sRadius*dKREIS_3);
 sRI=sR;
 if (auinit.sTeilung&S_9er)  
	Ellipse(hDC, sMpX-sR, sMpY-sR, sMpX+sR, sMpY+sR );
 if (!(auinit.sTeilung&S_3er))
	sRG=(short)((double)sRadius*dKREIS_STZ);
 sRHZ=sRG-(sRG-sRI)/2;
// +(short)((double)AstroFontRec.lfWidth/2);
 if (auinit.sTeilung&S_9er) {
	 dA=Radix->dStzGrad;
...
///5. Kreis (5 Grad)****************
 sRG=sR;
 sR=(short)((double)sRadius*dKREIS_5);
 sRI=sR;
 Ellipse(hDC, sMpX-sR, sMpY-sR, sMpX+sR, sMpY+sR );           
 ///6. Kreis (Aspekte)****************
...
{
 short  sA;
 short  sRD, sRHx, sRG=(short)((double)sRadius*dKREIS_STZ);
 short  sRH=(short)((double)sRadius*KREIS_ASP);
 short  sX, sY;
 double dA, dDezGrad;
 LOGPEN lpLogPen;
...
//					 "8", "9", "b", "11", "12" };
 ...
 lpLogPen.lopnStyle=PS_SOLID;
 lpLogPen.lopnWidth.x=1;
 lpLogPen.lopnColor=sColor[COL_HAUSER];
 DeleteObject(SelectObject(hDC, CreatePenIndirect(&lpLogPen)));
				  ...
				  char cAbstand, short sAnzahl)
{
 ...
 sRP=(short)((double)sRadius*dKREIS_STZ);
 sRPi=(short)((double)sRadius*dKREIS_PLANET);
...
short sDrawText(RADIX * Radix, RADIX* RadixV, DRAWITEMSTRUCT* Di, char * szBuffer, char cFlag)
{
 ...
 long  dIndex;
 char  szBuf[BUF_SIZE];
 short sSelHoro=(short)RadixV->cHoroTyp;
 RADIX* plRadix;

 if (RadixV->lRadixCount){
	plRadix=(RADIX*)RadixV->plRadix[0];
	sSelHoro=(short)plRadix->cHoroTyp;
 }/*endif*/
 ...
 case DRW_LB_BUFFER:
      ...
      if (dIndex&0x1000){
		  ...
		  sprintf(szBuffer,"%s%s %02dÙ%02d'%02d\" %2s", szLNANamen[(short)(dIndex&0xEFFF)],
					 (Radix->psPlanetTyp[(short)(dIndex&0xEFFF)]&P_TYP_RUCK)?"Ò":"",
 ...
 case DRW_LB_INDEX:
	 ...
	 if ( Radix->lPaintFlag&DRW_HAUS&&
		...
		SendMessage(Radix->hWndLBG, LB_ADDSTRING, 0, (LPARAM)DRW_LB_HAUS);
	 	for (sA=0; sA<12; sA++){
			 SendMessage(Radix->hWndLBG, LB_ADDSTRING, 0,
					(LPARAM)sA|(long)Radix->psPlanetTyp[0]<<16);
		 } /* endfor */
	 } /* endif */
...
//    	  sprintf(szBuffer, "Breite: %.2f", plRadix->rFix.dBreite);
 ...
 case DRW_LBD_INDEX:
	 if (sSelHoro==TYP_SYNASTRIE)
		SendMessage(Radix->hWndLBD, LB_ADDSTRING, 0, (LPARAM)(sSelHoro+1)^0xFFFFFFFF);
     else
 ...
 case DRW_LBA_BUFFER:
      ...
      vSetColor(Di);
      if (dIndex&0xF0000000)
		   sprintf(szBuffer,"%s%s in %s %s %s%s in %s",
				   szLNANamen[(short)(dIndex&0xFF)],
				   (Radix->psPlanetTyp[(short)(dIndex&0xFF)]&P_TYP_RUCK)?"Ò":"",
...
short sDrawAspekte(HDC hDC, RADIX * Radix, RADIX* RadixV, char cPen  )
{
 ...
 if ( Radix->lPaintFlag&DRW_HAUS&&auinit.sAspekte&S_HAUS ) {
...
if (Radix->lPaintFlag&DRW_REFRESH) {                  //Repaint Radix
   if (Radix->hBitmap) {
...
DeleteObject(SelectObject(hDC, CreatePenIndirect(&lpLogPen)));
sRet=sRadius+sDruber-2;
Ellipse(hDC, sMpX-sRet, sMpY-sRet, sMpX+sRet, sMpY+sRet );
lpLogPen.lopnColor=sColor[COL_RADIX];
DeleteObject(SelectObject(hDC, CreatePenIndirect(&lpLogPen)));
...
// hDC=(HDC)CreateMetaFile(NULL);
 if (Radix->lPaintFlag&DRW_TEXT) {
	SendMessage(Radix->hWndLBD,	LB_RESETCONTENT, 0 , 0L);
	...
	if (Radix->sLB&LB_A) {
		...
		if (Radix->sLB&LB_S)
		   SetWindowText(GetDlgItem(Radix->hWnd, PB_ASPEKTE), ASPEKTE_TXT);
		else
		   SetWindowText(GetDlgItem(Radix->hWnd, PB_ASPEKTE), TASPEKTE_TXT);
	 } else {
	...
	if (Radix->sLB&LB_G) {
		...
		if (Radix->sLB&LB_S)
		   SetWindowText(GetDlgItem(Radix->hWnd, PB_POSITION), S2POS_TXT);
        else
		   SetWindowText(GetDlgItem(Radix->hWnd, PB_POSITION), TPOS_TXT);
	} else {
		sDrawText(Radix, Radix, 0, 0, DRW_LB_INDEX);
		if (Radix->sLB&LB_S)
		   SetWindowText(GetDlgItem(Radix->hWnd, PB_POSITION), S1POS_TXT);
        else
		   SetWindowText(GetDlgItem(Radix->hWnd, PB_POSITION), POS_TXT);
	} /*endelse*/
	Radix->lPaintFlag^=DRW_TEXT;
 }/* endif*/
...
//		 (char*)Radix->pszName, (char*)Radix->pszOrt);
 ...
 sRadix(hDC, Radix);
// DeleteObject(SelectObject(hDC, CreateSolidBrush(sColor[COL_HINTER])));
...
// Radix->cStzIndex=0;
 ...
 Radix->rFix.cGultig=MAIN_VERSION;
 Radix->lPaintFlag=-1;
